import { Schemable2C, memoize } from 'io-ts/Schemable'
import { always, array, boolean, char, Nat, number, oneof, record, Source, string, tuple } from './source'
...
export function createSchemable(params: CreateSchemableParams = {}): Schemable2C<URI, Nat> {
  const schemable:  Schemable2C<URI, Nat>  = {
    ...
    partial,
    record: (codomain) => n => schemable.string(n).flatMap(key => codomain(n).map(value => ({ [key]: value }))),
    intersect: (right) => (left) => n => left(n).flatMap(l => right(n).map(r => ({ ...l, ...r }))),
    sum: (_tag => <A>(members: { [K in keyof A]: Source<Nat, A[K]>; }) => oneof(...members)) as Schemable2C<URI, Nat>['sum'],
    lazy,
  }
...
function convertToPartial<K extends PropertyKey, V>(key: K, value: Source<Nat, V>): [K, Source<Nat, V | undefined>] {
